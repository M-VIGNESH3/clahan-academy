...
2. `Clahan Academy Namespace` - 18 edges
3. `compilerOptions` - 16 edges
4. `Table Definitions & DDL Schema` - 16 edges
5. `🎓 Clahan Academy V2` - 15 edges
6. `🎓 Clahan Academy V2` - 15 edges
7. `bootstrap.sh script` - 13 edges
8. `Clahan-Academy Online Exam Platform Infrastructure (Active-Standby DR)` - 13 edges
9. `values-prod.yaml (Prod Overrides)` - 13 edges
10. `compilerOptions` - 12 edges

## Surprising Connections (you probably didn't know these)
- `Nginx Reverse Proxy Config` --semantically_similar_to--> `Nginx + Certbot HTTPS Option`  [INFERRED] [semantically similar]
...
- **Proctoring Violation Rules Engine Pattern** — clahan_academy_overview_violation_rules_engine, clahan_academy_overview_no_face_detected, clahan_academy_overview_multiple_faces_detected, clahan_academy_overview_mobile_phone_detected, clahan_academy_overview_book_detected, clahan_academy_overview_tab_switch, clahan_academy_overview_fullscreen_exit [EXTRACTED 1.00]
- **CI/CD + GitOps Deployment Pipeline** — build_workflow_ci_pipeline, deploy_workflow_cd_pipeline, terraform_apply_workflow_pipeline, argocd_application_resource, readme_helm_chart [EXTRACTED 1.00]
- **Shared Deployment Template Pattern (Workload Identity, CSI Secrets, Pod Anti-Affinity)** — deployment_admin, deployment_ai, deployment_auth, deployment_exam [INFERRED 0.80]
- **AI/Code-Execution Service Mesh (Exam & Proctoring to AI Service to Ollama, Exam to Judge0)** — values_exam, values_proctoring, values_ai, values_ollama, values_judge0 [INFERRED 0.85]
- **Helm Environment Overlay Pattern (Chart + Dev/Prod Value Overrides)** — chart_clahanacademy, valuesdev_overrides, valuesprod_overrides [INFERRED 0.75]
- **Default-Deny Plus Layered Allow NetworkPolicies** — denyalldefault_denyall, ainodenetpol_ainode, appnodenetpol_appcommon, appnodenetpol_notification [INFERRED 0.85]
- **Frontend High-Availability Trio (Deployment + HPA + PDB)** — frontenddeployment_frontend, frontendhpa_frontend, frontendpdb_frontend [INFERRED 0.85]
- **Shared DB/JWT/Redis Secret Schema Across SecretProviderClasses** — adminsecretprovider_admin, authsecretprovider_auth, examsecretprovider_exam [INFERRED 0.90]
- **Azure Workload Identity Enabled Service Accounts** — admin-sa_serviceaccount, ai-sa_serviceaccount, auth-sa_serviceaccount, exam-sa_serviceaccount, notification-sa_serviceaccount, proctoring-sa_serviceaccount, student-sa_serviceaccount [INFERRED 0.85]
- **Service Accounts Without Dedicated Client ID** — frontend-sa_serviceaccount, judge0-sa_serviceaccount, ollama-sa_serviceaccount [INFERRED 0.85]
- **Proctoring-to-AI Service Secret Flow** — proctoring-secretprovider_secretproviderclass, proctoring-sa_serviceaccount, ai-svc_service [INFERRED 0.75]
- **Shared AI Node Pool Scheduling Pattern** — ai_deployment_ai_service, judge0_api_deployment_judge0_api, judge0_worker_deployment_judge0_worker, judge0_redis_deployment_judge0_redis, values_ai_config, values_judge0_config [INFERRED 0.85]
- **ArgoCD GitOps Deployment Flow** — application_clahan_academy_application, project_clahan_academy_appproject, chart_clahan_academy_chart, values_base_values [INFERRED 0.85]
- **Gateway API Ingress/Routing Pattern** — gateway_clahan_gateway, httproute_clahan_routes, values_ingress_config, values_gateway_config [INFERRED 0.80]
- **Azure Key Vault CSI Secret Sync Pattern** — adminsecretprovider_clahanadminsecrets, aisecretprovider_clahanaisecrets, authsecretprovider_clahanauthsecrets, examsecretprovider_clahanexamsecrets, frontendsecretprovider_clahanfrontendsecrets, notificationsecretprovider_clahannotificationsecrets, proctoringsecretprovider_clahanproctoringsecrets, studentsecretprovider_clahanstudentsecrets [INFERRED 0.85]
- **CPU-based Horizontal Pod Autoscaling Pattern** — authhpa_authservicehpa, examhpa_examservicehpa, frontendhpa_frontendservicehpa, proctoringhpa_proctoringservicehpa [INFERRED 0.80]
- **Zero-Trust Default-Deny + Explicit-Allow Network Policy Pattern** — defaultdeny_defaultdeny, appnetpol_appnetpol, ainetpol_ainetpol [INFERRED 0.85]
- **Azure Workload Identity ServiceAccount Pattern** — ai-sa_clahan-ai-sa, auth-sa_clahan-auth-sa, exam-sa_clahan-exam-sa, frontend-sa_clahan-frontend-sa, notification-sa_clahan-notification-sa, proctoring-sa_clahan-proctoring-sa, student-sa_clahan-student-sa [INFERRED 0.85]
- **ClusterIP Service Exposure Pattern** — admin-svc_admin-service, ai-svc_ai-service, auth-svc_auth-service, exam-svc_exam-service, frontend-svc_frontend-service, judge0-svc_judge0-service, notification-svc_notification-service, ollama-svc_ollama, proctoring-svc_proctoring-service, student-svc_student-service [INFERRED 0.80]
- **Gateway API Routing Flow** — gateway_clahan-gateway, gateway_clahan-routes, auth-svc_auth-service, admin-svc_admin-service, student-svc_student-service, exam-svc_exam-service, proctoring-svc_proctoring-service, notification-svc_notification-service, frontend-svc_frontend-service [EXTRACTED 1.00]
- **AI Node Pool Scheduling (dedicated tainted nodes)** — ai_deployment, judge0_api_deployment, judge0_redis_deployment, judge0_worker_deployment [INFERRED 0.85]
- **Shared clahan-config/clahan-secrets consumption** — admin_deployment, auth_deployment, exam_deployment, notification_deployment [INFERRED 0.85]
- **Judge0 Code Execution Stack** — judge0_api_deployment, judge0_redis_deployment, judge0_worker_deployment, judge0_api_service, judge0_redis_service [EXTRACTED 1.00]
- **Azure Workload Identity federation across service accounts** — notification_sa, proctoring_sa, student_sa, azure_workload_identity_concept [INFERRED 0.85]
...
Cohesion: 0.07
Nodes (29): dependencies, bcryptjs, bullmq, cors, dotenv, express, express-rate-limit, helmet (+21 more)
...
Cohesion: 0.11
Nodes (18): 1. Frontend Service (`frontend-service`), 2. Authentication Service (`auth-service`), 3. Administrator Service (`admin-service`), 4. Student Service (`student-service`), 5. Exam Service (`exam-service`), 6. Proctoring Service (`proctoring-service`), 7. Notification Service (`notification-service`), AI-Assisted Grading Workflow (+10 more)
...
Cohesion: 0.20
Nodes (9): 🗺️ 1. High-Level Architecture & Components, 📦 2. Microservice Topology, 🗄️ 3. Relational Database Schema, ⚙️ 7. Environment Configuration Matrix, Clahan Academy V2 — Complete System Documentation & Architecture Guide, code:mermaid (graph TD), code:mermaid (erDiagram), Gateway Proxy Configuration (+1 more)
...
Cohesion: 0.24
Nodes (7): AssessmentPreExamStepper(), AssessmentPreExamStepperProps, PreExamValidationStep, PreExamFlowController(), PreExamFlowControllerProps, AssessmentInstructions(), AssessmentInstructionsProps
...
Cohesion: 0.32
Nodes (8): Application Insights, Azure Monitor, Azure Front Door (Standard), Internet, Log Analytics Workspace, rg-clahan-academy-global (Shared, Central India), Architecture Diagram: Clahan Academy Multi-Region DR, User

### Community 78 - "Community 78"
Cohesion: 0.25
Nodes (8): 🔄 4. Core Workflows & Life-Cycles, A. Authentication & Verification, B. Pre-Exam Verification Handshake, C. Live Proctoring Rules Engine, code:mermaid (sequenceDiagram), D. Asynchronous Email Queueing (BullMQ), E. Code Sandbox Integration (Judge0), Proctoring Violation Rules Matrix
...
Cohesion: 0.29
Nodes (4): checks, endpoints, HealthResult, issues
...
Cohesion: 0.50
Nodes (3): code:hcl (module "function_app" {), Function App Module, Usage Example
...
Cohesion: 0.50
Nodes (3): code:hcl (module "postgres_primary" {), PostgreSQL Primary Module, Usage Example

### Community 105 - "Community 105"
Cohesion: 0.50
Nodes (3): code:hcl (module "postgres_replica" {), PostgreSQL Replica Module, Usage Example

### Community 106 - "Community 106"
Cohesion: 0.50
Nodes (4): cadvisor scrape job, node-exporter scrape job, Prometheus scrape configuration, prometheus scrape job (self)

### Community 107 - "Community 107"
Cohesion: 0.50
Nodes (3): code:hcl (module "redis" {), Redis Cache Module, Usage Example
...
- `Azure Container Apps (ACA)` → `Terraform IaC`  [AMBIGUOUS]
  azure_architecture.md · relation: conceptually_related_to
- `exams Table` → `exams.navigation_mode / submission_mode Fields`  [AMBIGUOUS]
  VERIFY.md · relation: conceptually_related_to
- `training-service Container` → `Repository Structure Listing`  [AMBIGUOUS]
  docker-compose.yml · relation: conceptually_related_to
- `placement-service Container` → `Repository Structure Listing`  [AMBIGUOUS]
  docker-compose.yml · relation: conceptually_related_to
- `report-service Container` → `Repository Structure Listing`  [AMBIGUOUS]
  docker-compose.yml · relation: conceptually_related_to
- `Student Service (K8s Service)` → `Clahan HTTPRoute`  [AMBIGUOUS]
  helm/clahan-academy/templates/httproute.yaml · relation: references
- `AI Models PVC` → `AI Service Deployment`  [AMBIGUOUS]
  helm/clahan-academy/templates/deployments/ai-deployment.yaml · relation: conceptually_related_to
- `AI Models PVC` → `AI Service Values Config`  [AMBIGUOUS]
  clahan-academy/templates/storage/ai-models-pvc.yaml · relation: conceptually_related_to
- `AI Models PVC` → `Base Helm Values (values.yaml)`  [AMBIGUOUS]
  clahan-academy/templates/storage/ai-models-pvc.yaml · relation: references
- `Ollama Models PVC` → `Base Helm Values (values.yaml)`  [AMBIGUOUS]
  clahan-academy/templates/storage/ollama-models-pvc.yaml · relation: references
- `Ollama Models PVC` → `Ollama Values Config`  [AMBIGUOUS]
  clahan-academy/templates/storage/ollama-models-pvc.yaml · relation: conceptually_related_to
- `Dev Values Overrides` → `Clahan Academy Namespace`  [AMBIGUOUS]
  helm/clahan-academy/values-dev.yaml · relation: conceptually_related_to
- `Prod Values Overrides` → `Clahan Academy Namespace`  [AMBIGUOUS]
  helm/clahan-academy/values-prod.yaml · relation: conceptually_related_to
- `AI Service` → `Namespace Split Between Helm Chart (clahan-academy) and Static Manifests (production)`  [AMBIGUOUS]
  helm/clahan-academy/templates/services/ai-svc.yaml · relation: rationale_for
- `Judge0 Service` → `Clahan Config ConfigMap`  [AMBIGUOUS]
  kubernetes/configmap.yaml · relation: references
- `Production Namespace` → `Namespace Split Between Helm Chart (clahan-academy) and Static Manifests (production)`  [AMBIGUOUS]
...
- **What is the exact relationship between `Azure Container Apps (ACA)` and `Terraform IaC`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **What is the exact relationship between `exams Table` and `exams.navigation_mode / submission_mode Fields`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **What is the exact relationship between `training-service Container` and `Repository Structure Listing`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **What is the exact relationship between `placement-service Container` and `Repository Structure Listing`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **What is the exact relationship between `report-service Container` and `Repository Structure Listing`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **What is the exact relationship between `Student Service (K8s Service)` and `Clahan HTTPRoute`?**
  _Edge tagged AMBIGUOUS (relation: references) - confidence is low._
- **What is the exact relationship between `AI Models PVC` and `AI Service Deployment`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._